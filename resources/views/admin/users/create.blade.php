...
@section('title', 'Tambah User/Admin Baru')
...
<div class="container-fluid py-5">
    <div style="max-width: 800px; margin: 0 auto;">
        ...
        {{-- Header --}}
        <div class="mb-5">
            <h2 class="fw-800 text-dark mb-1">Tambah User Baru</h2>
            <p class="text-secondary mb-0">Buat akun pengguna atau administrator sistem baru.</p>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-body p-5">
                ...
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="name" class="form-control border-0 bg-light py-3 rounded-3 @error('name') is-invalid @enderror" value="{{ old('name') }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-700 small text-secondary">Email</label>
                                <input type="email" name="email" class="form-control border-0 bg-light py-3 rounded-3 @error('email') is-invalid @enderror" value="{{ old('email') }}" required>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label fw-700 small text-secondary">No. Telepon</label>
                                <input type="text" name="phone" class="form-control border-0 bg-light py-3 rounded-3" value="{{ old('phone') }}" placeholder="+62 8xx xxxx xxxx">
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Section: Password --}}
                    <div class="mb-5">
                        <h6 class="fw-800 text-primary text-uppercase small letter-spacing-1 mb-4">Keamanan</h6>
                        
                        <div class="row g-4">
                            <div class="col-md-6">
                                <label class="form-label fw-700 small text-secondary">Kata Sandi</label>
                                <input type="password" name="password" class="form-control border-0 bg-light py-3 rounded-3 @error('password') is-invalid @enderror" required>
                                <small class="text-secondary mt-1 d-block">Minimal 8 karakter</small>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-700 small text-secondary">Konfirmasi Kata Sandi</label>
                                <input type="password" name="password_confirmation" class="form-control border-0 bg-light py-3 rounded-3" required>
                            </div>
                        </div>
                    </div>
