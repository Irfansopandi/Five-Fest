...
<head>
    ...
    <title>@yield('title', 'Owner') - Five Fest</title>
    ...
    <style>
        ...
        .sidebar .nav-link span { margin-left: 10px; }
        ...
        .stat-card__link:hover { color: white; }
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('owner.reports') }}"
           class="nav-link {{ request()->routeIs('owner.reports*') ? 'active' : '' }}">
            ...
            <span>Laporan dari Admin</span>
            @php
                $unreadCount = \App\Models\Report::where('status', 'unread')->count();
            @endphp
            @if($unreadCount > 0)
                <span class="badge bg-warning text-dark ms-auto rounded-pill" style="font-size:0.65rem;">
                    {{ $unreadCount }}
                </span>
            @endif
        </a>
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', function () {
        if (window.innerWidth > 768) {
            const isCollapsed = localStorage.getItem('ownerSidebarCollapsed') === 'true';
            if (isCollapsed) {
        ...
        if (alertContainer) {
            ...
            var message   = atob(alertContainer.dataset.alertMessage);
            Swal.fire({
