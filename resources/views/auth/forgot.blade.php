                ...
                <div class="card border-0 shadow-lg glass-card">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <div class="mb-4 text-center">
                            ...
                            <p class="text-muted small">Masukkan email yang terdaftar, kami akan mengirimkan 6 digit OTP.</p>
                        </div>

                        <form action="{{ route('password.email') }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label class="form-label fw-semibold small text-secondary">Email Terdaftar</label>
                                <input type="email" name="email" class="form-control form-control-lg bg-light border-0 shadow-sm @error('email') is-invalid @enderror" placeholder="email@anda.com" required style="border-radius: 12px;">
                                @error('email')
                                    <div class="invalid-feedback mt-1 small">
                            ...
                            <button type="submit" class="btn btn-primary btn-lg w-100 text-white fw-bold shadow-sm mb-3" style="border-radius: 12px;">
                                KIRIM OTP <i class="bi bi-send ms-2"></i>
                            </button>

                            <p class="text-center text-muted small mb-0">
                                Kembali ke <a href="{{ route('login') }}" class="text-primary fw-bold text-decoration-none">Halaman Login</a>
                            </p>
                        </form>
                    </div>
                </div>
